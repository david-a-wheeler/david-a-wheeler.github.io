...
int parse_var(int type)
{
        ...
        if (scantoken == SET_TOKEN)
        {
                ...
                if ((consttype = parse_constexpr(&constval, &constsize)))
                {
                         ...
                         */
                        idglobal_size(type, size, emit_data(type, consttype, constval, constsize));
                }
                else if (scantoken == OPEN_BRACKET_TOKEN)
                {
                        /*
...
int main(int argc, char **argv)
{
        ...
        if (argc > 1 && argv[1][0] == '-')
            emit_flags(EDASM);
        ctag = tag_new();
        idfunc_add("romcall",  7, ctag);
        idfunc_add("ROMCALL",  7, ctag);
        ctag = tag_new();
        idfunc_add("syscall",  7, ctag);
        idfunc_add("SYSCALL",  7, ctag);
        ctag = tag_new();
        idfunc_add("memset",   6, ctag);
        idfunc_add("MEMSET",   6, ctag);
        ctag = tag_new();
        idfunc_add("memcpy",   6, ctag);
        idfunc_add("MEMCPY",   6, ctag);
        ctag = tag_new();
        idfunc_add("cout",     4, ctag);
        idfunc_add("COUT",     4, ctag);
        ctag = tag_new();
        idfunc_add("cin",      3, ctag);
        idfunc_add("CIN",      3, ctag);
        ctag = tag_new();
        idfunc_add("prstr",    5, ctag);
        idfunc_add("PRSTR",    5, ctag);
        ctag = tag_new();
        idfunc_add("rdstr",    5, ctag);
        idfunc_add("RDSTR",    5, ctag);
        emit_header();
        if (parse_module())
        {
