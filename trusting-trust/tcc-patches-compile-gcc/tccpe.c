...
ST int pe_find_import(TCCState * s1, const char *symbol, void **paddr)
{
    int sym_index = find_elf_sym(s1->dynsymtab_section, symbol);
    if (0 == sym_index) {
        /* Hm, maybe it's '_symbol' instead of 'symbol' or '__imp__symbol' */
        char buffer[100];
        if (0 == memcmp(symbol, "__imp__", 7)) /* mingw 2.0 */
            symbol += 6;
        else
        if (0 == memcmp(symbol, "_imp___", 7)) /* mingw 3.7 */
            symbol += 6;
        else
...
{
    void *addr;
    pe_find_import(s1, symbol, &addr);
    if (addr && STT_OBJECT == type)
    {
        /* need to return a pointer to the address for data objects */
        dynarray_add(&pe_imp, &nb_pe_imp, addr);
        addr = &pe_imp[nb_pe_imp - 1];
    }
    return addr;
}
#endif
...
ST void pe_set_datadir(int dir, DWORD addr, DWORD size)
{
...
ST int pe_write_pe(struct pe_info *pe)
{
    ...
    pe_header.filehdr.NumberOfSections = sec_index;
...
ST struct import_symbol * pe_add_import(struct pe_info *pe, int sym_index)
{
    int i;
    int dll_index;
...
/* ------------------------------------------------------------- */
ST void pe_build_reloc (struct pe_info *pe, int *section_order, int section_count)
{
    ...
    for(;;)
    {
        ...
        if (count) /* store the last block and ready for a new one */
        {
            ...
            if (count & 1) section_ptr_add(pe->reloc, sizeof(WORD)), ++count;
...
/* ------------------------------------------------------------- */
ST void pe_relocate_rsrc (struct pe_info *pe, Section *s)
{
    Section *sr = s->reloc;
    DWORD voffset = s->sh_addr - pe->imagebase;
    Elf32_Rel *rel, *rel_end;
    ...
    for(rel = (Elf32_Rel *)sr->data; rel < rel_end; rel++)
        *(DWORD*)(s->data + rel->r_offset) += voffset;
}
...
ST int pe_check_symbols(struct pe_info *pe)
{
    ...
    for (sym_index = 1; sym_index < sym_end; ++sym_index)
    {
        ...
        if (sym->st_shndx == SHN_UNDEF)
        {
            ...
            int imp_sym = pe_find_import(pe->s1, name, NULL);
            struct import_symbol * is;

            if (0 == imp_sym) goto not_found;
            ...
            if (type == STT_FUNC)
            {
                ...
                else
                {
                    ...
                    *(WORD*)section_ptr_add(text_section, 8) = 0x25FF;
                    ...
                    put_elf_reloc(
                        symtab_section, text_section, offset + 2,
                        R_386_32, /*R_JMP_SLOT, */
                        is->iat_index
                        );
                ...
                /* tcc_realloc might have destroyed sym's address */
                sym = (Elf32_Sym*)symtab_section->data + sym_index;
        ...
        if (pe->s1->rdynamic && ELF32_ST_BIND(sym->st_info) != STB_LOCAL)
        {
 ...
 *  as written with 'windres.exe -O coff ...'.
 */

int pe_test_resource_file(struct pe_rsrc_header *p, int size)
{
    return
    size >= IMAGE_SIZEOF_FILE_HEADER + IMAGE_SIZEOF_SHORT_NAME
...
int pe_load_resource_file(TCCState *s1, FILE *fp)
{
    ...
    k = sizeof hdr;
    fseek (fp, 0, SEEK_SET);
    if (k != fread (&hdr, 1, k, fp))
        goto fail;

    if (!pe_test_resource_file(&hdr, k))
        goto fail;

    rsrc_section = new_section(s1, ".rsrc", SHT_PROGBITS, SHF_ALLOC);

    k = hdr.sectionhdr.SizeOfRawData;
    ptr = section_ptr_add(rsrc_section, k);

    fseek (fp, hdr.sectionhdr.PointerToRawData, SEEK_SET);
    if (k != fread (ptr, 1, k, fp))
        goto fail;

    fseek (fp, hdr.sectionhdr.PointerToRelocations, SEEK_SET);
    for (i = 0; i < hdr.sectionhdr.NumberOfRelocations; ++i)
    {
        k = sizeof rel;
        if (k != fread (&rel, 1, k, fp))
            goto fail;

        put_elf_reloc(
            symtab_section, rsrc_section, rel.offset,
            R_386_NONE,
            0
            );
    }
    return 0;
fail:
    error("could not load resource file");
    return -1;
}
...
int pe_load_def_file(TCCState *s1, FILE *fp)
{
    ...
    return 0;
fail:
    error("file format (.def) not recognized");
    return -1;
}
